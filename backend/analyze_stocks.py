...
# ── CONFIG ───────────────────────────────────────────────────────────────────
MIN_RESULTS         = 10      # safety gate — abort if fewer than this succeed
CAD_MIN_MARKETCAP   = 100e6   # $100M CAD
CAD_MIN_VOLUME      = 50_000
USD_MIN_MARKETCAP   = 500e6   # $500M USD
USD_MIN_VOLUME      = 100_000
MAX_CAD_TICKERS     = 150     # cap to keep workflow under 6h limit
MAX_USD_TICKERS     = 150
FETCH_DELAY         = 1.2     # seconds between yfinance calls
REQUEST_TIMEOUT     = 20
...
def fetch_tmx_tickers():
    """
    Fetch all TSX + TSXV tickers from the TMX public JSON API.
    Endpoint returns paginated results; we walk all pages.
    Falls back to a curated seed list if the API is unreachable.
    """
    tickers = []
    headers = {
    ...
    # Deduplicate, sort by implied importance, cap
...
def fetch_nasdaq_nyse_tickers():
    """
    Fetch all NASDAQ + NYSE tickers from the official NASDAQ FTP data feed.
    ...
    try:
        for url, sym_col, test_col in feeds:
            ...
            for i, row in enumerate(reader):
                ...
                # Skip test issues, warrants, ETFs with $ or ^
                if test_issue == "Y":
                    continue
    ...
    # Liquidity pre-filter using yfinance fast_info (batch)
...
def _liquidity_filter_usd(tickers):
    """
    Batch-check market cap using yfinance fast_info to filter
    out micro/nano caps before full analysis. Processes in chunks.
    """
    qualified = []
    chunk_size = 50
