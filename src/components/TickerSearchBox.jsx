...
/* ── Result modal rendered at document.body level ── */
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: '1.2rem', color: 'var(--color-primary)', letterSpacing: '0.06em' }}>
              {result.stock.ticker}
            </span>
            {result.stock.name && (
              <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', fontWeight: 500 }}>{result.stock.name}</span>
            )}
        ...
        <div style={{
          ...
          {[
            ...
            <div key={label} style={{
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: '1.25rem', color: color || 'var(--color-text)', letterSpacing: '-0.01em' }}>{value}</div>
        ...
        {result.stock.horizons && (
          <div style={{
            ...
            {Object.entries(HORIZON_LABELS).map(([hKey, hLabel]) => {
              ...
              return (
                <div key={hKey} style={{
                  flex: '1 1 80px', textAlign: 'center', padding: '8px',
...
export default function TickerSearchBox({ cadAll, usdAll, cadSorted, usdSorted, horizon }) {
  ...
  const suggestions = useMemo(() => {
    ...
    return all.filter(s => s.ticker?.toUpperCase().startsWith(q)).slice(0, 8)
  }, [query, cadAll, usdAll])
  ...
  const handleSearch = useCallback((ticker) => {
    ...
    setModalOpen(true)
  }, [query, cadAll, usdAll, cadSorted, usdSorted])
  ...
  return (
    <div ref={wrapperRef} style={{ maxWidth: '720px', margin: '0 auto 32px', position: 'relative' }}>
      ...
      {/* Autocomplete — rendered via portal above everything */}
      {showSuggestions && (
        <SuggestionsPortal
      ...
      {/* Result card — rendered as a centered modal via portal */}
      {modalOpen && result && h && (
        <ResultModal
          result={result}
          h={h}
