...
export default function StockCard({ stock, horizon, darkMode, livePrice }) {
  ...
  /* Live price overlay — falls back to snapshot if prices.json not yet available */
    ...
    <div
      ...
      style={{
        borderColor: expanded
          ? `color-mix(in oklch,${rs.border} 60%,var(--color-border))`
          : undefined,
        boxShadow: expanded
          ? `var(--shadow-md), 0 0 20px ${rs.glow}`
          : undefined,
      }}
    >
      {/* Forensic banner */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {/* LIVE badge — only shown when prices.json is loaded */}
              {isLive && (
                <span style={{
            ...
            <div style={{ fontSize: '0.7rem', color: 'var(--color-text-faint)',
                          fontFamily: 'var(--font-body)', marginTop: 
